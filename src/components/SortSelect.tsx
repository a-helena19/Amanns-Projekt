...
export default function SortSelect() {
  ...
  return (
    <Select.Select.Root value={currentSort} onValueChange={(value) => {
      ...
      <Select.Select.Trigger className="w-24 px-2 py-1 rounded bg-white hover:bg-gray-50 cursor-pointer flex items-center justify-between">
            ...
            <Select.Select.List>
              {sortOptions.map((option) => (
                <Select.Select.Item
                  key={option.value}
                  value={option.value}
                  className="px-4 py-2 text-lg cursor-pointer hover:bg-gray-100 text-gray-600"
                >
